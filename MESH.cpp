...
void timer(int)
{
		...
		while (1)
		{
			...
			if (G_TIME >= pathtime[n][i] && G_TIME < pathtime[n][i + 1])
			{
				x_position[n] = nodeArray[agentpath_node[n][i] - 1][0]
					+ (nodeArray[agentpath_node[n][i + 1] - 1][0] 
						- nodeArray[agentpath_node[n][i] - 1][0])
					* (G_TIME - pathtime[n][i]) / ((pathtime[n][i + 1]) - pathtime[n][i]);

				y_position[n] = nodeArray[agentpath_node[n][i] - 1][1]
					+ (nodeArray[agentpath_node[n][i + 1] - 1][1] 
						- nodeArray[agentpath_node[n][i] - 1][1])
					* (G_TIME - pathtime[n][i]) / ((pathtime[n][i + 1]) - pathtime[n][i]);
				...
				//agentdose[n] += 1.1 / 60.0 * 4.6E-6 * conc[xx][yy]; // 1.1m3/h, 4.6E-6mSv/Bq, KAERI
				//totaldose += 1.1 / 60.0 * 4.6E-6 * conc[xx][yy];
	...
	greenbytime.push_back(AGENT_NUMBER - numred - numorange);
	...
	if (HYSPLIT_STEP == SIMULATION_STEP)
	{
		...
		if (CHECK_BYTIME) {
			...
			std::cout << greenbytime.size() << std::endl;
		...
		sprintf(output, "./log/data/check.txt");
		outFile = fopen(output, "w");
		fprintf(outFile, "\n");
		fclose(outFile);
